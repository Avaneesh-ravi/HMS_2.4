    ...
    if (submissions.length > 0) {
      ...
      for (const r of submissions) {
        ...
        fullResponses.push({
          ...
          overallRating: (rawR.length > 0 ? (rawR.find(x => String(x.question_text || '').toLowerCase().includes('overall'))?.rating || rawR[0]?.rating) : 5) ? parseInt((rawR.find(x => String(x.question_text || '').toLowerCase().includes('overall'))?.rating || rawR[0]?.rating || 5), 10) : (avgRating ? Math.round(avgRating) : 5),
          consolidatedRating: avgRating || 5,
